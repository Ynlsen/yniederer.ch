...
export async function generateStaticParams() {
  return getAllSlugs("de").map((slug) => ({ slug }));
}
  ...
  try {
    ...
    return {
      ...
      openGraph: {
        ...
        locale: "de",
        url: `https://yniederer.ch/de/projects/${slug}`,
      },
      alternates: {
        canonical: `https://yniederer.ch/de/projects/${slug}`,
        languages: {
...
export default async function DeBlogPage({ params }: PageProps) {
  const { slug } = await params;

  let post;
  try {
    post = await getPostBySlug(slug, "de");
  } catch {
    notFound();
  }
  ...
  const articleSchema = {
    ...
    inLanguage: "de",
    mainEntityOfPage: `https://yniederer.ch/de/projects/${slug}`,
  };
